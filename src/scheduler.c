...
/**
 * @brief High-frequency loop for isolated distributed execution (SPSC path).
 */
...
int scheduler_start(void) {
    ...
    for (uint32_t i = 0; i < g_state.config.num_workers; i++) {
        ...
        int (*target_loop)(void *) = (g_state.config.topology == TOPOLOGY_SHARED_QUEUE) ? 
                                      worker_shared_queue_loop : worker_distributed_queue_loop;
